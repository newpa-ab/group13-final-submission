...
static void spawn_powerup(StrikerEngine_t* engine, float x, float y, uint8_t source_type)
{
    ...
    for (i = 0; i < STRIKER_MAX_POWERUPS; i++) {
        ...
        if (!p->active) {
            p->x = x;
            p->y = y;
            p->vx = ((float)((int16_t)Random_U16(50) - 25) / 100.0f);
            p->vy = STRIKER_POWERUP_FALL_SPEED;
...
static void spawn_forced_powerup(StrikerEngine_t* engine, float x, float y, uint8_t type)
{
    ...
    for (i = 0; i < STRIKER_MAX_POWERUPS; i++) {
        ...
        if (!p->active) {
            p->x = x;
            p->y = y;
            p->vx = ((float)((int16_t)Random_U16(70) - 35) / 100.0f);
            p->vy = STRIKER_POWERUP_FALL_SPEED;
            p->ttl = STRIKER_POWERUP_TTL;
            p->type = type;
            p->active = 1U;
...
{
    uint16_t hp = (uint16_t)(STRIKER_BOSS_BASE_HP + (engine->boss_kills * STRIKER_BOSS_HP_STEP));
...
static void fire_boss_scatter(StrikerEngine_t* engine)
{
    ...
    uint8_t count = (engine->boss_kills >= 2U) ? 7U : 5U;
...
{
    uint8_t i;
    uint8_t count = (engine->boss_kills >= 1U) ? 3U : 1U;
    ...
    for (i = 0; i < count; i++) {
        ...
        float speed = 3.0f + ((float)engine->boss_kills * 0.08f);
...
static uint8_t update_boss(StrikerEngine_t* engine)
{
    ...
    if (engine->boss_attack_timer > 0U) {
        ...
        if (engine->boss_attack_phase == 0U) {
            ...
            engine->laser_warning_timer = 36U;
        }
        engine->boss_attack_phase = (uint8_t)((engine->boss_attack_phase + 1U) % 3U);
        engine->boss_attack_timer = (uint16_t)(84U - ((engine->boss_kills > 5U) ? 30U : (engine->boss_kills * 6U)));
    }
...
static void update_powerups(StrikerEngine_t* engine)
{
    ...
    for (i = 0; i < STRIKER_MAX_POWERUPS; i++) {
        ...
        p->vx *= 0.95f;
        p->vy *= 0.96f;
        if (engine->magnet_timer == 0U && p->vy < STRIKER_POWERUP_FALL_SPEED) {
            p->vy = STRIKER_POWERUP_FALL_SPEED;
        }
