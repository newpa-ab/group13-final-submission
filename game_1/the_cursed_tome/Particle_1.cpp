...
void ParticleSystem::Draw(int x, int halfScale)
{
	...
	int8_t horizon = SceneRenderer::GetHorizon(x);
	...
	for (Particle& p : particles)
	{
		if (p.IsActive())
		{
			...
			if (outX >= 0 && outY >= 0 && outX < DISPLAY_WIDTH - 1 && outY < DISPLAY_HEIGHT - 1 && halfScale >= SceneRenderer::wBuffer[outX])
			{
...
void ParticleSystemManager::Draw()
{
	for (ParticleSystem& system : systems)
	{
		if(system.IsActive())
		{
			...
			if(SceneRenderer::TransformAndCull(system.worldX, system.worldY, screenX, screenW))
			{
				QueuedDrawable* drawable = SceneRenderer::CreateQueuedDrawable((uint8_t)screenW);
				if(drawable)
				{
