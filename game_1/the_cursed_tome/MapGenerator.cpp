...
void MapGenerator::SplitMap(uint8_t x, uint8_t y, uint8_t w, uint8_t h, uint8_t doorX, uint8_t doorY)
{
	...
	if (splitVertical)
	{
		...
		if (splitAttempts > 0)
		{
			...
			{
				Map::SetCell(i, y + splitSize, CellType::TombWall);
			}
	...
	else if (splitHorizontal)
	{
		...
		if (splitAttempts > 0)
		{
			...
			{
				Map::SetCell(x + splitSize, j, CellType::TombWall);
			}
...

	{
		...
		// Add decorations
		{
			...
			if (w == h && w >= 7 && h >= 7)
			{
				Map::SetCell(x + 1, y + 1, CellType::TombWall);
				Map::SetCell(x + w - 2, y + 1, CellType::TombWall);
				Map::SetCell(x + w - 2, y + h - 2, CellType::TombWall);
				Map::SetCell(x + 1, y + h - 2, CellType::TombWall);
			}
		}
	}
}
...
void MapGenerator::Generate()
{
	...
	for (int y = 0; y < Map::height; y++)
	{
		for (int x = 0; x < Map::width; x++)
		{
			...
			Map::SetCell(x, y, isEdge ? CellType::TombWall : CellType::Empty);
		}
	}
	...
	// Find any big open spaces
	{
		...
		while (hasOpenSpaces)
		{
			...
			if (space > 6)
			{
				...
				{
					Map::SetCell(x + n, y + 2, CellType::TombWall);
					Map::SetCell(x + 2, y + n, CellType::TombWall);
					Map::SetCell(x + n, y + space - 3, CellType::TombWall);
					Map::SetCell(x + space - 3, y + n, CellType::TombWall);
				}
			}
		}
	}
	...
	// Add torches
	{
		...
		while (attempts > 0 && toSpawn > 0)
		{
			...
			if (Map::GetCellSafe(x, y) == CellType::Empty)
			{
				...
				if(info.count == 1 && GetDistanceToCellType(x, y, CellType::Brazier) > minSpacing)
				{
					Map::SetCell(x, y, CellType::Brazier);
					toSpawn--;
					attempts = 255;
				}
			}
	...
	// Add blocking decorations
	{
		uint8_t attempts = 255;
		uint8_t toSpawn = 255;
		CellType cellType = CellType::CanopicJar;
		uint8_t minSpacing = 3;
	...
	Map::SetCell(1, 1, CellType::EntrySeal);
	Map::SetCell(Map::width - 3, Map::height - 3, CellType::TombGate);
	...
	if(false)
	{
		...
		while (attempts > 0)
		{
			...
			{
				Map::SetCell(x, y, CellType::StoneTablet);
				break;
			}
	...
	else if(TombGame::floor == 1)
	{
		Map::SetCell(2, 2, CellType::StoneTablet);
	}
	...
	// Add treasure / items
	{
		uint16_t attempts = 65535;
		uint8_t toSpawn = 8;
		CellType cellType = CellType::RelicCasket;
		uint8_t minSpacing = 3;
		...
		while (attempts > 0 && toSpawn > 0)
		{
			...
			{
			case 0:
				cellType = CellType::LifeEssence;
				break;
			case 1:
				cellType = CellType::RelicShard;
				break;
			case 2:
				cellType = CellType::RelicCasket;
				break;
			case 3:
				cellType = CellType::SunIdol;
				break;
			case 4:
				cellType = CellType::RuneTablet;
				break;
			}
			...
			if (Map::GetCellSafe(x, y) == CellType::Empty)
			{
				...
				&& GetDistanceToCellType(x, y, CellType::EntrySeal) > minExitSpacing
				&& GetDistanceToCellType(x, y, CellType::TombGate) > minExitSpacing)
				{
					Map::SetCell(x, y, cellType);
					toSpawn--;
					attempts = 255;
