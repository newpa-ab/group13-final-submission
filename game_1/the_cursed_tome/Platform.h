...
public:
	...
	static uint16_t GetSessionSeed();

	static void ExpectLoadDelay();
	
	static void FillScreen(uint8_t col);
	static void PutPixel(uint8_t x, uint8_t y, uint8_t colour);
	static void SetDrawTint(uint8_t tint);
	static uint8_t GetDrawTint();
	static void SetPixelTint(uint8_t x, uint8_t y, uint8_t tint);
	static void TintRect(uint8_t x, uint8_t y, uint8_t w, uint8_t h, uint8_t tint);
