...
void TitleFlow::DrawEnteringLevel()
{
	...
	if (TombGame::floor == 1)
	{
		PrintCentered(PSTR("SECTOR 1 OF 2"), 3);
		PrintCentered(PSTR("BREACH THE TOMB"), 4);
		PrintCentered(PSTR("THE TOME IS AWAKE"), 5);
		PrintCentered(PSTR("ITS SIGNAL WANTS"), 6);
		PrintCentered(PSTR("THE OPEN SKY"), 7);
	}
	else
	{
		PrintCentered(PSTR("SECTOR 2 OF 2"), 3);
		PrintCentered(PSTR("SEAL THE CORE"), 4);
		PrintCentered(PSTR("THE SIGNAL GROWS"), 5);
		PrintCentered(PSTR("FOLLOW IT BEFORE"), 6);
		PrintCentered(PSTR("OTHERS ANSWER"), 7);
	}
}	
