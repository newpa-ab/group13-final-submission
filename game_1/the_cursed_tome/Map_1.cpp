...
void Map::DrawMinimap()
{
	...
	uint8_t playerCellX = TombGame::player.x / CELL_SIZE;
	uint8_t playerCellY = TombGame::player.y / CELL_SIZE;
	...
	for (uint8_t x = 0; x < minimapWidth; x++)
	{
		...
		for (uint8_t y = 0; y < minimapHeight; y++)
		{
			...
			{
				Platform::PutPixel(outX, outY, (TombGame::globalTickFrame & 3) ? COLOUR_BLACK : COLOUR_WHITE);
			}
			else
			{
